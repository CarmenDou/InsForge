...
- `tool_choice` (string | object, optional) - Controls tool usage: `'auto'`, `'none'`, `'required'`, or `{ type: 'function', function: { name: string } }`
- `parallel_tool_calls` (boolean, optional) - Allow the model to call multiple tools in parallel
...
if (message.tool_calls) {
  ...
  const orderStatus = await getOrderStatus(args.order_id);

  // Step 3: Send the tool result back
  const finalResponse = await insforge.ai.chat.completions.create({
    model: 'openai/gpt-4',
    messages: [
      { role: 'user', content: 'What is the status of order #12345?' },
  ...
  // "Order #12345 is currently being shipped and will arrive on Friday."
}
```
